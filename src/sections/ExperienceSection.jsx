...
export function ExperienceSection() {
  ...
  return (
    <section className="bg-[#19182d] px-[5.5vw] py-16 text-white max-md:px-4 max-md:py-12" id="experience">
      <h2 className="mb-12 text-[clamp(32px,4vw,52px)] font-black tracking-normal">
        Industry Experience
      </h2>
      <div className="relative mx-auto mb-14 grid min-h-[300px] max-w-[1180px] grid-cols-2 items-center gap-8 before:absolute before:left-0 before:right-0 before:top-1/2 before:h-1.5 before:-translate-y-1/2 before:rounded-full before:bg-gradient-to-r before:from-[#ff664d] before:via-[#7f5cff] before:to-[#25c99c] max-lg:min-h-0 max-lg:grid-cols-1 max-lg:before:hidden">
        {EXPERIENCE.map((job, index) => {
          const isActive = activeExperienceIndex === index;

          return (
            <button
              className={`relative z-10 mx-auto grid w-[190px] justify-items-center rounded-[18px] border bg-white p-5 text-center text-[#19192d] shadow-[0_18px_38px_rgba(0,0,0,0.18)] transition-all duration-300 ${
            ...
            >
              ...
              <div className="mb-3 grid h-12 w-12 place-items-center rounded-xl bg-[#f7f8fa] text-sm font-black text-[var(--accent)]">
                {job.mark}
              </div>
              <h3 className="text-base font-black">{job.company}</h3>
      ...
      <div className="mx-auto max-w-[1040px] rounded-[22px] border border-white/10 bg-white/[0.04] p-8 shadow-[0_24px_70px_rgba(0,0,0,0.2)] max-md:p-6">
        <h3 className="mb-5 text-2xl font-black tracking-normal">
          {activeExperience.company}{" "}
          <span className="text-base font-black text-white/35">
            Roles & Responsibilities
          </span>
        </h3>
        <ul className="grid gap-3 pl-5 text-base leading-8 text-white/70">
